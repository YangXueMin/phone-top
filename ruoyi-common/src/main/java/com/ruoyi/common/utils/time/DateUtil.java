...
public class DateUtil {
    ...
    public static double getDifferHour(Date startDate, Date endDate) {
        ...
        BigDecimal two = new BigDecimal(hours);
        //保留一位数
        hours = two.setScale(1, BigDecimal.ROUND_HALF_UP).doubleValue();
        return hours;
    }
     ...
     * @return
     */
    public static long day(Date startDate, Date endDate) {
     ...
     * @throws
     */
    public static int month(Date start, Date end) {
     ...
     * @throws
     */
    public static int year(Date start, Date end) {
        Calendar bef = Calendar.getInstance();
        Calendar aft = Calendar.getInstance();
        bef.setTime(start);
        aft.setTime(end);
        return aft.get(Calendar.YEAR) - bef.get(Calendar.YEAR) + 1;
    }
}
